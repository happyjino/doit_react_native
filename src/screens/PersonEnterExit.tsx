...
import { useToggle, useTransformStyle, useAnimatedValues, useAnimatedValue, useStyle } from '../hooks';
...
const PersonEnterExit: FC<PersonProps> = ({ person, deletePressed }) => {
  ...
  const iconAnimations = useMemo(() => 
    iconAnimValues.map((animValue) =>
      Animated.spring(animValue, { useNativeDriver: true, toValue: !started ? 1: 0 })
    )
    , [started])
  
  const enterAnimation = useCallback(() => {
    Animated.sequence([
      Animated.timing(leftToRightAnimValue, { useNativeDriver: true, toValue: 1, duration: 1 * 1000, easing: Easing.bounce }),
      Animated.spring(opacityAnimValue, { useNativeDriver: true, toValue: 1 }),
      Animated.timing(topBottomAnimValue, { useNativeDriver: true, toValue: 1, duration: 1 * 1000, easing: Easing.circle }),
      ...iconAnimations
    ]).start(toggleStarted)
  }, [])
  ...
  const exitAnimation = useCallback(() => {
    Animated.sequence([
      ...iconAnimations,
      Animated.parallel([
        Animated.spring(topBottomAnimValue, { useNativeDriver: true, toValue: 0 }),
        Animated.spring(opacityAnimValue, { useNativeDriver: true, toValue: 0}),
      ]),
      Animated.timing(leftToRightAnimValue, { useNativeDriver: true, toValue: 0, duration: 0.3 * 1000}),
    ]).start(deletePressed)
  }, [started])
  ...
  const topOrBottomTransformStyle = useTransformStyle({
    translateX: interpolate(topBottomAnimValue, started ? [400, 0] : [-400, 0])
  }, [started])
  ...
  return (
    <Animated.View style={[styles.view, enterLeaveTransformStyle]}>
      ...
      <View style={[styles.rightView]}>
        ...
        <Animated.Image style={[styles.image]} source={{uri: person.image}} />
